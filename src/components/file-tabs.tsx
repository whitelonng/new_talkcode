import { Circle, Copy, FileText, Globe, MessageSquarePlus, X } from 'lucide-react';
...
export function FileTabs({
  ...
  const handleOpenInBrowser = async (filePath: string) => {
    if (onOpenInBrowser) {
      await onOpenInBrowser(filePath);
    }
    setContextMenuTabIndex(null);
  };
  ...
  const handleOpenInExplorer = async (filePath: string) => {
    try {
      await fileExplorerService.openPath(filePath);
    } catch (error) {
          ...
          return (
            <ContextMenu key={file.path}>
              ...
              <ContextMenuContent className="w-48">
                ...
                {onOpenInBrowser && canOpenInBrowser(file.path) && (
                  <ContextMenuItem
                    onClick={() => handleOpenInBrowser(file.path)}
                    className="cursor-pointer"
                  >
